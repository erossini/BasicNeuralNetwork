...
var outputs = Vector<double>.Build.DenseOfEnumerable(new List<double>() { 1, 1, 1, 0 });//NAND f(x,y)=x*x +y*y+3
//inputs.Dump("Inputs");
//outputs.Dump("Outputs");
int epochs = 3000;
double learningRate = 0.1;
nn = Train(nn, epochs, learningRate, inputs, outputs);
nn = Predict(nn, Vector<double>.Build.DenseOfArray(new double[] { 1, 1 }));//
nn.Prediction.Dump();
