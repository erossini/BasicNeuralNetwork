...
//MODEL f(x,y) = x*x+y*y
var nn = CreateNeuralNetwork(new List<int>() { 2, 6, 6, 1 });
...
inputs = inputs.Map(x=>x/inputMax);
outputs = outputs.Map(x=>x/outputMax);
int epochs = 20000;
double learningRate = 0.1;
nn = Train(nn, epochs, learningRate, inputs, outputs);
nn = Predict(nn, Vector<double>.Build.DenseOfArray(new double[] { 4/inputMax, 9/inputMax }));//
(4*4+9*9).Dump();
//nn.Prediction.Dump();
var prediction = nn.Prediction*outputMax;//denormalizing the normalized prediction...because nn is using normalized train/test data
prediction.Dump();
